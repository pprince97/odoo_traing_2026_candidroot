...
class FleetPortal(CustomerPortal):
    ...
    def portal_fleet(
        ...
        if request.env.user.partner_id:
            driver_domain = [('driver_id', '=', request.env.user.partner_id.id)]
        base_domain = driver_domain
        if driver_domain and not Fleet.search_count(driver_domain):
            base_domain = []
        ...
        return request.render('library_management.portal_fleet_template', values)
