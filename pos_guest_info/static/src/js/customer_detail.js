/** @odoo-module **/

import {Component, useState} from "@odoo/owl";
import {Dialog} from "@web/core/dialog/dialog";
import { useService } from "@web/core/utils/hooks";

export class CustomerDetail extends Component {
    static components = {Dialog};
    static template = "pos_guest_info.CustomerDetail";

    setup() {
        console.log("Set up is load");

        this.state = useState({
            no_of_male: this.props.no_of_male || 0,
            no_of_female: this.props.no_of_female || 0,
            no_of_guest: this.props.no_of_guest || 0,

            guests: Array.from({length: this.props.no_of_guest || 0}, () => ({
                age: "",
                country: "",
                gender: "",
            })),
        });
        console.log("no_of_guest ::::>>>   ", this.state.no_of_guest)
    }

    // updateGuestCount() {
    //     if (this.state.no_of_male >= 0 && this.state.no_of_female >= 0) {
    //         this.state.no_of_guest = Number(this.state.no_of_male) + Number(this.state.no_of_female);
    //     }
    //     else {
    //         alert("Person is not negative")
    //         this.state.no_of_male = 0
    //         this.state.no_of_female = 0
    //         this.state.no_of_guest = 0
    //     }
    // }


    async confirm() {
        const data = {
            guests: this.state.guests,
            no_of_male: this.state.no_of_male,
            no_of_female: this.state.no_of_female,
            no_of_guest: this.state.no_of_guest,
        };
        if (this.props.confirm) {
            await this.props.confirm(data);
        }
        this.props.close();
    }

    cancel() {
        this.props.close({confirmed: false});
    }
}

export class CustomerDetailRequired extends Component {
    static components = {Dialog};
    static template = "pos_guest_info.CustomerDetailRequired";

    setup() {
        console.log("Set up is load");

        this.state = useState({
            no_of_male: this.props.no_of_male || 0,
            no_of_female: this.props.no_of_female || 0,
            no_of_guest: this.props.no_of_guest || 0,

            guests: Array.from({length: this.props.no_of_guest || 0}, () => ({
                age: "",
                country: "",
                gender: "",
            })),
        });
        console.log("no_of_guest ::::>>>   ", this.state.no_of_guest)
    }

    // updateGuestCount() {
    //     if (this.state.no_of_male >= 0 && this.state.no_of_female >= 0) {
    //         this.state.no_of_guest = Number(this.state.no_of_male) + Number(this.state.no_of_female);
    //     }
    //     else {
    //         alert("Person is not negative")
    //         this.state.no_of_male = 0
    //         this.state.no_of_female = 0
    //         this.state.no_of_guest = 0
    //     }
    // }


    async confirm() {
        for (let i = 0; i < this.state.guests.length; i++) {
            const guest = this.state.guests[i];

            // Check if any field is empty or undefined
            if (!guest.age || !guest.country || !guest.gender) {
                // Use Odoo's notification service to tell the user what's wrong
                this.env.services.notification.add(
                    `Please fill in all details for Guest #${i + 1}`,
                    {type: "danger"}
                );
                return; // Stop the function here so the dialog doesn't close
            }
        }

        const data = {
            guests: this.state.guests,
            no_of_male: this.state.no_of_male,
            no_of_female: this.state.no_of_female,
            no_of_guest: this.state.no_of_guest,
        };
        if (this.props.confirm) {
            await this.props.confirm(data);
        }
        this.props.close();
    }

    cancel() {
        this.props.close({confirmed: false});
    }
}